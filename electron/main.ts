...
function configureAppSettings() {
  try {
    ...
    if (!isPackaged) {
      ...
      // Probe for hardware GPU only when the user has not explicitly set a value.
      let hasHardwareGpu = false
      if (enableGpuExplicitly === null) {
        try {
          // Windows: use WMI query to list video controllers.
          if (process.platform === 'win32') {
            try {
              const out = String(
                execSync('wmic path win32_VideoController get Name /format:list', { timeout: 2000 })
              )
              const lines = out
                .split(/\r?\n/)
                .map((s: string) => s.trim())
                .filter(Boolean)
              const names = lines
                .filter((l: string) => l.toLowerCase().startsWith('name='))
                .map((l: string) => l.slice(5).trim())
              const bad = ['microsoft basic display adapter', 'virtual', 'vbox', 'vmware']
              for (const n of names) {
                const ln = n.toLowerCase()
                if (!bad.some((b) => ln.includes(b))) {
                  hasHardwareGpu = true
                  break
                }
              }
            } catch (e) {
              // wmic may be unavailable; assume no dedicated GPU.
            }
          } else if (process.platform === 'darwin') {
            try {
              const out = String(execSync('system_profiler SPDisplaysDataType', { timeout: 2000 }))
              if (/Chipset|Intel|NVIDIA|AMD|Apple/gi.test(out)) hasHardwareGpu = true
            } catch (e) {}
          } else if (process.platform === 'linux') {
            try {
              const out = String(execSync('lspci -nn | grep -i vga', { timeout: 2000 }))
              if (out && out.trim().length > 0) hasHardwareGpu = true
            } catch (e) {}
          }
        } catch (e) {}
      }

      // Determine whether to disable GPU: disable when the user explicitly opted out,
      // or when no hardware GPU was detected and the user provided no explicit choice.
      // In dev, always disable GPU to avoid driver issues.
      const shouldDisableGpu =
        enableGpuExplicitly === false || (enableGpuExplicitly === null && !hasHardwareGpu)
      if (shouldDisableGpu) {
        try {
...
function getPreferredThemeBg(): string {
  try {
    if (existsSync(STATE_FILE)) {
      ...
      const theme = raw && raw.theme ? String(raw.theme) : ''
      if (theme === 'light') return DEFAULT_LIGHT_THEME['--app-bg']
      if (theme === 'dark') return DEFAULT_DARK_THEME['--app-bg']
      if (theme === 'system') {
        try {
...
function createWindow(initialNote?: string, emptyWindow?: boolean, focusedMode?: boolean): any {
  ...
  win.webContents.on(
    ...
    (_event: any, errorCode: number, errorDescription: string, validatedURL: string) => {
      console.error(
        `[main] window failed to load — code=${errorCode} desc="${errorDescription}" url="${validatedURL}"`
      )
      ipcMain.removeListener('notara:renderer-ready', readyHandler)
      clearTimeout(showTimeout)
      try {
  ...
  try {
    const rawState = existsSync(STATE_FILE) ? JSON.parse(readFileSync(STATE_FILE, 'utf-8')) : {}
    const savedBounds = rawState?.windows?.[winId]?.bounds
    if (savedBounds && savedBounds.width >= 400 && savedBounds.height >= 300) {
  ...
  const saveBounds = () => {
    ...
    boundsTimer = setTimeout(() => {
      try {
        const raw = existsSync(STATE_FILE) ? JSON.parse(readFileSync(STATE_FILE, 'utf-8')) : {}
...
function registerIPCHandlers_impl() {
  ...
  ipcMain.handle('fs:write-note', async (_e: any, filename: string, content: string) => {
    ...
    try {
      ...
      writeFileSync(join(NOTES_DIR, safeFilename), safeContent, 'utf-8')
    } catch (err) {
  ...
  ipcMain.handle('fs:write-note-atomic', async (_e: any, filename: string, content: string) => {
    ...
    try {
      if (!existsSync(NOTES_DIR)) mkdirSync(NOTES_DIR, { recursive: true })
      writeFileSync(tmp, safeContent, 'utf-8')
      renameSync(tmp, target)
    } catch (err) {
      console.error('[main] fs:write-note-atomic error:', err)
      // Clean up temp file if it survived the failed write
      try {
  ...
  ipcMain.handle('fs:rename-note', async (_e: any, oldFilename: string, newFilename: string) => {
    ...
    if (!existsSync(oldPath)) throw new Error('Source file does not exist')
  ...
  ipcMain.handle('state:load', async () => {
    try {
      ...
      console.error('[main] state:load error:', err)
  ...
  ipcMain.handle('theme:list', async () => {
    try {
      ...
      return readdirSync(THEMES_DIR).filter((f) => f.endsWith('.json'))
    } catch {
      return []
    }
  })
  ...
  ipcMain.handle('theme:write', async (_e: any, name: string, data: Record<string, string>) => {
    ...
    writeFileSync(join(THEMES_DIR, normalized), JSON.stringify(safeTheme, null, 2), 'utf-8')
  ...
  ipcMain.handle('fs:save-version', async (_e: any, filename: string, content: string) => {
    ...
    if (!existsSync(noteVersionDir)) mkdirSync(noteVersionDir, { recursive: true })
  ...
  ipcMain.handle('fs:list-versions', async (_e: any, filename: string) => {
    const safeFilename = requireNoteFilename(filename)
    const noteVersionDir = join(VERSIONS_DIR, safeFilename)
    if (!existsSync(noteVersionDir)) return []
    return readdirSync(noteVersionDir)
  ...
  ipcMain.handle('fs:write-recovery', async (_e: any, filename: string, content: string) => {
    const safeFilename = requireNoteFilename(filename)
    const safeContent = sanitizeNoteContent(content)
    const dir = join(APP_ROOT, 'recovery')
    if (!existsSync(dir)) mkdirSync(dir, { recursive: true })
    writeFileSync(join(dir, safeFilename), safeContent, 'utf-8')
  })
  ...
  ipcMain.handle('fs:read-recovery', async (_e: any, filename: string) => {
    const safeFilename = requireNoteFilename(filename)
    const p = join(APP_ROOT, 'recovery', safeFilename)
