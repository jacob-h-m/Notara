...
export default function App() {
  ...
  useEffect(() => {
    ...
    setRecentFiles((prev) => {
      ...
      // Persist to state.json (fire and forget)
      loadState()
        .then((s) => saveState({ ...s, recentFiles: next }))
        .catch(() => {})
      return next
    })
  }, [tabManager.activeTab])
  ...
  useEffect(() => {
    ...
    // Another window renamed a note — open new tab first, then close old
    const cleanupFileRenamed = window.api.onFileRenamed((oldFilename, newFilename) => {
      ...
      if (isOpen) {
        const wasActive = tabManager.activeTab === oldFilename
        void tabManager.openTab(newFilename).then(() => {
          if (wasActive) tabManager.setActiveTab(newFilename)
          tabManager.closeTab(oldFilename)
        })
      }
    })
    return () => {
  ...
  // Build FTS index (and note content map for tags) when note list is available
  useEffect(() => {
    ...
    void (async () => {
      ...
      // Update tag content map as a batch — triggers useMemo in useTags
  ...
  const handleContentChangeWithIndex = useCallback(
    (filename: string, value: string) => {
      ...
      // Always update the content map so tags re-index on every keystroke
  ...
  useEffect(() => {
    ...
    return window.api.onStateChanged((incoming) => {
      ...
      // Apply language change immediately — i18next re-renders all consumers
      if (incoming.language && i18n.language !== incoming.language) {
        void i18n.changeLanguage(incoming.language).catch(() => {})
      }
    })
  }, [])
            ...
            >
              ...
              <div className="max-h-80 overflow-y-auto">
                <VersionHistoryPanel
                  ...
                  onRestore={(content) => {
                    if (tabManager.activeTab)
                      tabManager.updateContent(tabManager.activeTab, content)
