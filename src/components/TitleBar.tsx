...
export default function TitleBar({ activeNote, menuContent }: TitleBarProps) {
  ...
  useEffect(() => {
    ...
    const onResize = async () => {
      if (mounted) setIsMax(await window.api.isMaximized())
    }
  ...
  return (
    ...
    >
      ...
      >
        ...
        {menuContent && <div className="titlebar-menu flex items-center px-1">{menuContent}</div>}
      </div>

      {/* ── Centre: drag fill + active note name ───────────────────── */}
      <div className="drag-region flex-1 h-full flex items-center justify-center overflow-hidden">
        <span
          className="truncate text-xs text-muted select-none px-4"
          style={{ pointerEvents: 'none' }}
        >
          {activeNote ? stemFilename(activeNote) : ''}
        </span>
      </div>
      ...
      >
        ...
        {/* Close — ✕ */}
        <button
          onClick={() => window.api.requestAppClose()}
