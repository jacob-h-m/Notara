...
function LanguageTab({
  ...
  return (
    <div>
      <CollapsibleSection title="Interface Language">
        <p className="mb-3 text-xs text-muted">
          Choose the display language for the application. Changes apply immediately.
        </p>
            ...
            <button
              ...
              className={`flex w-full items-center justify-between rounded-lg px-3 py-2.5 text-sm transition-colors ${
                language === lang.code
                  ...
                  : 'text-on-surface hover:bg-surface-800'
              }`}
              style={
                language === lang.code
                  ? ({
        ...
        <p className="mt-3 text-[11px] text-muted">
          Want to help translate Notara?{' '}
          <button
            onClick={() =>
              window.api.openExternal('https://github.com/jacob-h-m/Notara').catch(console.error)
            }
  ...
  return (
    <div>
      <CollapsibleSection title="All Tags">
        <p className="mb-3 text-xs text-muted">
          Tags are derived from #hashtags in your notes. Click a tag to expand its note list.
        </p>
        {tagIndex.length === 0 ? (
          <p className="py-4 text-center text-xs text-muted">
            No tags found. Add #hashtags to notes to see them here.
          </p>
        ) : (
          <>
            {/* Search filter */}
                ...
                {filtered.map(({ tag, count, notes }) => (
                  <div key={tag} role="listitem">
                    ...
                    <button
                      ...
                      className="flex w-full items-center justify-between rounded-lg px-3 py-2 text-sm transition-colors hover:bg-surface-800"
                      aria-expanded={expandedTag === tag}
                    >
                      <span className="font-mono text-[12.5px]" style={{ color: 'var(--accent)' }}>
                        #{tag}
                      </span>
                      <div className="flex items-center gap-2">
...
export default function SettingsModal({
  ...
  const [localLanguage, setLocalLanguage] = useState<string>(state.language ?? 'en')
  ...
  useEffect(() => {
    ...
    setLocalLanguage(state.language ?? 'en')
  }, [state.settings, state.editor, state.theme, state.language])
  ...
  function handleLanguageChange(code: LanguageCode) {
    ...
    // Apply immediately — react-i18next re-renders all consumers
  ...
  return (
    ...
    >
      ...
      >
        {/* Left tab column */}
        <nav
          className="settings-nav flex w-40 shrink-0 flex-col py-4"
        ...
        >
          ...
          >
            {TABS.map((tabItem) => (
              ...
              >
                {t(`settings.tabs.${tabItem.id}`, { defaultValue: tabItem.label })}
              </button>
            ))}
          </div>
        </nav>
        ...
        <div className="flex flex-1 flex-col overflow-hidden">
          ...
          <div className="settings-header flex items-center justify-between px-6 py-4">
            ...
            >
              {t(`settings.tabs.${tab}`, {
                defaultValue: TABS.find((tb) => tb.id === tab)?.label ?? '',
              })}
            </h2>
            <button
              onClick={onClose}
              className="btn-icon settings-close p-1"
              aria-label={t('settings.close', { defaultValue: 'Close settings' })}
            >
              <svg
                width="14"
                height="14"
          ...
          >
            ...
            {tab === 'language' && (
              <LanguageTab language={localLanguage} onLanguage={handleLanguageChange} />
            )}
            {tab === 'tags' && (
              <TagsTab tagIndex={tagIndex} onOpenNote={onOpenNote} onClose={onClose} />
            )}
            {tab === 'about' && <AboutTab />}
