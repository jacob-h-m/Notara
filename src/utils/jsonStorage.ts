...
export async function loadState(): Promise<AppState> {
  try {
    ...
    if (!raw) return { ...DEFAULT_STATE, editor: { ...DEFAULT_STATE.editor }, settings: { ...DEFAULT_STATE.settings } }
    return {
      ...DEFAULT_STATE,
      ...raw,
    ...
    return { ...DEFAULT_STATE, editor: { ...DEFAULT_STATE.editor }, settings: { ...DEFAULT_STATE.settings } }
  }
}
...
export async function renameStateReferences(oldFilename: string, newFilename: string): Promise<AppState> {
  const state = await loadState()
  const replace = (items: string[]) => items.map((item) => item === oldFilename ? newFilename : item)
  const next: AppState = {
    ...state,
